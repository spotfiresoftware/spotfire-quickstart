# Spotfire QuickStart on Azure Virtual Machines

## Overview

This **Spotfire QuickStart on Azure Virtual Machines** shows 
how to automatically deploy the [Spotfire platform](https://www.spotfire.com/) 
on [Azure Virtual Machines](https://azure.microsoft.com/en-us/products/virtual-machines).
...
1. Create the required infrastructure components using [Terraform](https://www.terraform.io/) templates:
   ...
   - Jump-host VMs: Azure Linux Virtual Machine instances.
   - Spotfire VMs: Azure Linux Virtual Machine instances.
   - Spotfire database: Azure Database for PostgreSQL flexible server.
...
**Note**: This deployment has been verified with Azure Linux Virtual Machine instances with Debian 12.

The following diagram shows the deployed environment by this _Spotfire QuickStart on Azure Virtual Machinese_:

![Spotfire QuickStart on Azure Virtual Machines](docs/images/spotfire-arch-azure-basic-agw-az.png)
...
3. Install the required configuration management applications.
   ...
   to manually install and configure Azure, Terraform and Ansible clients in your launcher.
...
1. If not already done, sign in into Azure with the Azure CLI:
    ```bash
...
7. Open the Spotfire server web UI, accessible via the Azure Application Gateway address: `http://<your-azure-app-gw-ip-address>`.
