...
SFS_FRONTEND_PORT=${SFS_FRONTEND_PORT:-8080}
SFS_REGISTRATION_PORT=${SFS_REGISTRATION_PORT:-9080}
SFS_BACKEND_PORT=${SFS_BACKEND_PORT:-9443}

#SFS_RECONCILER_PORT=${SFS_RECONCILER_PORT:-8081}
#LIBADMIN_USER=${LIBADMIN_USER:-libadmin}
#SFS_CONFIGURED=${SFS_CONFIGURED:-/opt/spotfire/vol/spotfireserver-configured}
...
config_server_ports() {
  ...
  ${SPOTFIRE_HOME}/configure -s ${SFS_FRONTEND_PORT} -r ${SFS_REGISTRATION_PORT} -b ${SFS_BACKEND_PORT}
  show_result $?
  echo
}
...
get_address() {
  ...
  if [ "${ONLY_IPS}" == "true" ] ; then
    echo "nodemanager.use.ip=true" >> ${SPOTFIRE_HOME}/nm/config/nodemanager.properties
...
exists_bootstrap_file() {
  [[ -s ${SPOTFIRE_HOME}/tomcat/webapps/spotfire/WEB-INF/bootstrap.xml ]]
}

create_bootstrap_config() {
  echo "Creating the bootstrap file database connection configuration..."
  IP_ADDRESSES=''
  echo "nodemanager.use.ip=false" >> ${SPOTFIRE_HOME}/nm/config/nodemanager.properties
...
#  SERVER_IP_LIST=$(hostname)
  ...
  #echo "nodemanager.host.names=" >> ${SPOTFIRE_HOME}/nm/config/nodemanager.properties
...
exists_default_config() {
  [[ -s ${SPOTFIRE_HOME}/tomcat/spotfire-bin/configuration.xml ]]
}

create_default_config() {
  if [[ -s ${SPOTFIRE_HOME}/tomcat/spotfire-bin/configuration.xml ]] ; then
...
config_java_env() {
  ...
  sed -i -e "s/Xmx4096M/Xmx${JAVA_XMX_MEMORY}/" ${SPOTFIRE_HOME}/tomcat/bin/setenv.sh
  show_result $?
  echo
}
